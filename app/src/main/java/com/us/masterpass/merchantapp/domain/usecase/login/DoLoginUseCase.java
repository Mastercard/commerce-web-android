 ...
 */
public class DoLoginUseCase extends UseCase<DoLoginUseCase.RequestValues, DoLoginUseCase.ResponseValue> {
        ...
        if (values.username.length() > 0 && values.password.length() > 0) {
            mMasterpassExternal.doLogin(values.getUsername(), values.getPassword(),
                    new MasterpassDataSource.LoadDataLoginCallback() {
                        @Override
                        public void onDataLogin(LoginObject loginObject) {
                            ...
                            ResponseValue responseValue = new
                                    ResponseValue(loginObject, true);
