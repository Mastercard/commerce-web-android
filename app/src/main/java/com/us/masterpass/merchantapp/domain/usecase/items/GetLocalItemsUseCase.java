 ...
 */
public class GetLocalItemsUseCase
    extends UseCase<GetLocalItemsUseCase.RequestValues, GetLocalItemsUseCase.ResponseValue> {
   ...
   */
  public GetLocalItemsUseCase(@NonNull ItemRepository itemsRepository, Context context) {
    mContext = context;
  }
  ...
  private List<Item> itemsToList(String responseString) {
    ...
    try {
      ...
      for (int i = 0; i < jsonArray.length(); i++) {
        ...
        String image = mImageEndPoint.substring(0, extensionStartIndex);
        item.setImage(image);
