...
public class ConfirmExpressTransactionUseCase extends UseCase<ConfirmExpressTransactionUseCase.RequestValues, ConfirmExpressTransactionUseCase.ResponseValue> {
    ...
    protected void executeUseCase(final RequestValues values) {
        ...
        values.masterpassConfirmationObject.setDoCheckoutAmount(completeAmount);
        values.masterpassConfirmationObject.setDoCheckoutSupressShipping(
                SettingsSaveConfigurationSdk.getInstance(mContext).getSupressShipping()
        );

        mMasterpassExternal.expressCheckout(values.masterpassConfirmationObject, new MasterpassDataSource.LoadDataConfirmationCallback() {
            @Override
