...
public class MasterpassExternalDataSource implements MasterpassDataSource {
    ...
    @Override
    public void getDataConfirmation(Map<String, Object> checkoutData,
                                    boolean expressCheckoutEnable,
                                    LoadDataConfirmationCallback callback) {
        final RestApi restApi = new RestApi();
        String response = restApi.getDataConfirmation(checkoutData, expressCheckoutEnable);
        if (response == null) {
            ...
            try {
                ...
                if (jsonObject.getBoolean(CommerceConstants.STATUS_API_CALL)) {
                    JSONObject paymentData = new JSONObject(
                            jsonObject.getString(CommerceConstants.RESPONSE_API_CALL)
                    );

                    if (checkoutData != null) {
                        callback.onDataConfirmation(
                                transformResponse(
                                        paymentData,
                                        checkoutData.get(
                                                CommerceConstants.API_CALL_TRANSACTION_ID).toString()
                                ),
    ...
    @Override
    public void sendConfirmation(MasterpassConfirmationObject masterpassConfirmationObject, LoadDataConfirmationCallback callback) {
        final RestApi restApi = new RestApi();
    ...
    @Override
    public void expressCheckout(MasterpassConfirmationObject masterpassConfirmationObject, LoadDataConfirmationCallback callback) {
        final RestApi restApi = new RestApi();
        String response = restApi.setExpressCheckout(masterpassConfirmationObject);
        if (response == null) {
            callback.onDataNotAvailable();
        } else {
            try {
                ...
                if (jsonObject.getBoolean(CommerceConstants.STATUS_API_CALL)) {
                    ...
                    callback.onDataConfirmation(transformResponseCheckout(paymentData, masterpassConfirmationObject.getPreCheckoutTransactionId()), false);
                } else {
                    callback.onDataNotAvailable();
                }
            } catch (JSONException e) {
    ...
    @Override
    public void doLogin(String username, String password, LoadDataLoginCallback callback) {
        final RestApi restApi = new RestApi();
        String response = restApi.doLogin(username, password);
        if (response == null) {
            callback.onDataNotAvailable();
        } else {
            try {
    ...
    @Override
    public void getPairingId(Map<String, Object> checkoutData, PairingIdCallback callback) {
        final RestApi restApi = new RestApi();
        checkoutData.put(CommerceConstants.PAIRING_WITHOUT_CHECKOUT_FLOW, true);
        String response = restApi.getPairingId(checkoutData);
        if (response == null) {
            ...
            try {
                ...
                if (jsonObject.getBoolean(CommerceConstants.STATUS_API_CALL)) {
                    ...
                    if (paymentData.has(PAIRING_ID_OUTPUT) && paymentData.getJSONObject(PAIRING_ID_OUTPUT).has(PAIRING_ID)) {
                            MasterpassSdkCoordinator.savePairingId(JsonParser.getString(
                                    paymentData.getJSONObject(PAIRING_ID_OUTPUT),
                                PAIRING_ID, "")
                            );
                    }
                    callback.onPairing();
     ...
     */
    private MasterpassConfirmationObject transformResponse(
        ...
        try {
            if (paymentData.has(PAYMENT_DATA_OUTPUT)) {
                masterpassConfirmation.setCardBrandId(JsonParser.getString(
                        paymentData.getJSONObject(PAYMENT_DATA_OUTPUT).getJSONObject("card"),
                        "brandId", ""));
                ...
                if (JsonParser.getJSONObject(paymentData.getJSONObject(PAYMENT_DATA_OUTPUT).toString(), SHIPPING_ADDRESS).has(LINE1)) {
                    String line1 = JsonParser.getString(paymentData
                            .getJSONObject(PAYMENT_DATA_OUTPUT)
                            .getJSONObject(SHIPPING_ADDRESS),
                        LINE1, "");

                    String line2 = JsonParser.getString(paymentData
                            .getJSONObject(PAYMENT_DATA_OUTPUT)
                            .getJSONObject(SHIPPING_ADDRESS),
                        "city", "");

                    String line3a = JsonParser.getString(paymentData
                            .getJSONObject(PAYMENT_DATA_OUTPUT)
                            .getJSONObject(SHIPPING_ADDRESS),
                        SUB_DIVISION, "");

                    String line3b = JsonParser.getString(paymentData
                            .getJSONObject(PAYMENT_DATA_OUTPUT)
                            .getJSONObject(SHIPPING_ADDRESS),
                        POSTAL_CODE, "");
                ...
                masterpassConfirmation.setPairingId(JsonParser.getString(paymentData.getJSONObject(PRE_CHECKOUT_DATA_OP), PAIRING_ID, ""));
                masterpassConfirmation.setPreCheckoutTransactionId(JsonParser.getString(paymentData.getJSONObject(PRE_CHECKOUT_DATA_OP), "precheckoutTransactionId", ""));
                MasterpassSdkCoordinator.savePairingId(JsonParser.getString(
                        paymentData.getJSONObject(PRE_CHECKOUT_DATA_OP),
                    PAIRING_ID, "")
                );
                List<MasterpassPreCheckoutCardObject> listCard = new ArrayList<>();
                JSONArray cards = paymentData.getJSONObject(PRE_CHECKOUT_DATA_OP).getJSONObject("preCheckoutData").getJSONArray("cards");
                ...
                JSONArray shipping = paymentData.getJSONObject(PRE_CHECKOUT_DATA_OP).getJSONObject("preCheckoutData").getJSONArray("shippingAddresses");
                for (int i = 0; i < cards.length(); i++) {
                    ...
                    mpcso.setPreRecipientName(JsonParser.getString(JsonParser.getJSONObject(jsonObject.toString(), "recipientInfo"), "recipientName", ""));
                    mpcso.setPreRecipientPhone(JsonParser.getString(JsonParser.getJSONObject(jsonObject.toString(), "recipientInfo"), "recipientPhone", ""));
    ...
    private MasterpassConfirmationObject transformResponseCheckout(JSONObject paymentDataResponse, String transactionId) {
        MasterpassConfirmationObject masterpassConfirmation = new MasterpassConfirmationObject();
        try {
            JSONObject paymentData = paymentDataResponse.getJSONObject("ExpressCheckoutOutput").getJSONObject("paymentData");

            masterpassConfirmation.setCardBrandId(JsonParser.getString(
                    paymentData.getJSONObject("card"),
                    "brandId", ""));
            masterpassConfirmation.setCardBrandName(JsonParser.getString(
                    paymentData.getJSONObject("card"),
                BRAND_NAME, ""));
            ...
            masterpassConfirmation.setShippingCity(JsonParser.getString(
                    paymentData.getJSONObject(SHIPPING_ADDRESS),
                    "city", ""));
            masterpassConfirmation.setExpressCheckoutEnable(false);
            masterpassConfirmation.setCompleteTransactionId(transactionId);
            masterpassConfirmation.setCartId(MasterpassSdkCoordinator.getGeneratedCartId());
