...
public class CartConfirmationPairingAddressAdapter extends PagerAdapter {
   ...
   * @param item the item
   */
  public CartConfirmationPairingAddressAdapter(Context context,
      List<MasterpassPreCheckoutShippingObject> item) {
  ...
  @Override public Object instantiateItem(ViewGroup container, final int position) {
    ...
    line1.setText(new StringBuilder().append(item.getPreLine1())
        .append(" ")
        .append(item.getPreLine2())
        .toString());
    line2.setText(new StringBuilder().append(item.getPreLine1()));
    line3.setText(new StringBuilder().append(item.getPreSubdivision())
        .append(" ")
        .append(item.getPrePostalCode())
        .toString());
