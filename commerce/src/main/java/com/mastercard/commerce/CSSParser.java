...
class CSSParser {
  ...
  private static class SimpleSelector {
    ...
    public void addAttrib(String attrName, AttribOp op, String attrValue) {
      if (attribs == null) attribs = new ArrayList<Attrib>();
      attribs.add(new Attrib(attrName, op, attrValue));
    }

    public void addPseudo(String pseudo) {
      if (pseudos == null) pseudos = new ArrayList<String>();
      pseudos.add(pseudo);
    }
    ...
    @Override public String toString() {
      ...
      if (combinator == Combinator.CHILD) {
        sb.append("> ");
      } else if (combinator == Combinator.FOLLOWS) sb.append("+ ");
  ...
  static class Ruleset {
    ...
    void addAll(Ruleset rules) {
      ...
      if (this.rules == null) this.rules = new ArrayList<Rule>(rules.rules.size());
  ...
  private static class Selector {
    ...
    public void add(SimpleSelector part) {
      if (this.selector == null) this.selector = new ArrayList<SimpleSelector>();
  ...
  private static boolean mediaMatches(List<MediaType> mediaList, MediaType rendererMediaType) {
    for (MediaType type : mediaList) {
      if (type == MediaType.all || type == rendererMediaType) return true;
    }
    return false;
  }
   ...
   */
  static boolean ruleMatch(Selector selector, SVG.SvgElementBase obj) {
  ...
  private static boolean ruleMatch(Selector selector, int selPartPos,
      List<SVG.SvgContainer> ancestors, int ancestorsPos, SVG.SvgElementBase obj) {
    // We start at the last part of the selector and loop back through the parts
    // Get the next selector part
    ...
    } else //if (sel.combinator == Combinator.FOLLOWS)
    {
      ...
      SVG.SvgElementBase prevSibling =
          (SVG.SvgElementBase) obj.parent.getChildren().get(childPos - 1);
  ...
  private static boolean ruleMatchOnAncestors(Selector selector, int selPartPos,
      List<SVG.SvgContainer> ancestors, int ancestorsPos) {
    SimpleSelector sel = selector.get(selPartPos);
  ...
  private static int getChildPosition(List<SVG.SvgContainer> ancestors, int ancestorsPos,
    ...
    for (SVG.SvgObject child : obj.parent.getChildren()) {
      if (child == obj) return childPos;
      childPos++;
    }
    return -1;
  }

  private static boolean selectorMatch(SimpleSelector sel, List<SVG.SvgContainer> ancestors,
      int ancestorsPos, SVG.SvgElementBase obj) {
    ...
    if (sel.tag != null) {
      ...
      if (sel.tag.equalsIgnoreCase("G")) {
        if (!(obj instanceof SVG.Group)) return false;
      }
    ...
    if (sel.attribs != null) {
      for (Attrib attr : sel.attribs) {
        if (attr.name == ID) {
          if (!attr.value.equals(obj.id)) return false;
        } else if (attr.name == CLASS) {
          if (obj.classNames == null) return false;
          if (!obj.classNames.contains(attr.value)) return false;
        } else {
    ...
    if (sel.pseudos != null) {
      for (String pseudo : sel.pseudos) {
        if (pseudo.equals("first-child")) {
          if (getChildPosition(ancestors, ancestorsPos, obj) != 0) return false;
        } else {
          return false;
        }
      }
