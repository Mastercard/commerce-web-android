...
public class SettingsAdapter extends BaseAdapter {
  ...
  @Override public View getView(int i, View view, ViewGroup viewGroup) {
    ...
    if (item.getName().equalsIgnoreCase(SettingsConstants.ITEM_ENABLE_PAYMENT_METHOD)) {
      ...
      if (VIEW_CARD == viewType) {
        ...
        if (item.getName().equalsIgnoreCase(SettingsConstants.ITEM_EXPRESS) || item
            .getName().equals(SettingsConstants.ENABLE_WEB_CHECKOUT)) {
          if (!SettingsFragment.isLogged && !item
              .getName().equals(SettingsConstants.ENABLE_WEB_CHECKOUT)) {
            viewHolder.enableSW.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
    ...
    if (VIEW_SWITCH == viewType) {
      if (SettingsFragment.isLogged || item.getName()
          ...
          .equalsIgnoreCase(SettingsConstants.ITEM_ENABLE_PAYMENT_METHOD)
              || item.getName()
  ...
  private void pairingCall(){
    try {
      ...
      Toast.makeText(context,R.string.sdk_reinitialize,Toast
          .LENGTH_LONG)
          .show();
    }
  }
