...
public class TwoLeggedOAuthHelper {
  ...
  void addCommonRequestParameters(String baseUrl, String httpMethod) throws OAuthException {
    ...
    if (parameters.getOAuthSignature().length() == 0) {
      String baseString =
          OAuthUtil.getSignatureBaseString(baseUrl, httpMethod, parameters.getBaseParameters());
   ...
   */
  String addParametersAndRetrieveHeader(String requestUrl, String httpMethod)
    ...
    headerParams.add(OAuthParameters.OAUTH_SIGNATURE_KEY, parameters.getOAuthSignature());

    // Add all other base parameters to the header
    for (Map.Entry<String, String> e : parameters.getBaseParameters().entrySet()) {
    ...
    return (new StringBuilder()).append(OAuthParameters.OAUTH_KEY)
        .append(" ")
        .append(headerParams.toString())
        .toString();
  }
}
